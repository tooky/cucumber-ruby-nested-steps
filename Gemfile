...
unless ENV['CUCUMBER_USE_RELEASED_GEMS']
  ...
  if File.exist?(cucumber_path) && !ENV['CUCUMBER_USE_GIT']
    gem "cucumber", path: cucumber_path, branch: "remove-nested-steps-to-plugin"
  else
    gem "cucumber", :git => "git://github.com/cucumber/cucumber-ruby.git", branch: "remove-nested-steps-to-plugin"
  end
